...
unsigned int ROLLS = 10; //TODO - "They see me rolling!"
...
void SetTransitionMatrix()
{
	...
	for (int i = 0; i < TransitionMatrix.rows(); i++)
	{
		...
		{
			TransitionMatrix(i, i + 1) = 1; // You're about to win, assuming the world doesn't end before your next turn...
		}
		else if (i == TransitionMatrix.rows() - 1)
		{
			TransitionMatrix(i, i) = 1; //YOU WIN!!! And the world didn't end!
		}
	}

	///Part c - comment out for null matrix
	///the two sections can be combined, but for readability I left them seperated
	if (size > 99)
	{
		//Ladder from 3 to 19
